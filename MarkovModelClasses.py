...
class PatientStateMonitor:
    ...
    def __init__(self, parameters):
        ...
        self._timeToStroke = 0        # time to have had a stroke
    ...
    def update(self, k, next_state):
        ...
        if next_state in [P.HealthStats.DEATH]:
            self._survivalTime = (k+0.5)*self._delta_t  # corrected for the half-cycle effect (death in middle of cicle)
    ...
    def get_time_to_Stroke(self):
        """ returns the patient's time to Stroke """
        # return time to Post Stroke  only if the patient had a Stroke
        if self._ifStroke:
...
class CohortOutputs:
    def __init__(self, simulated_cohort):
        ...
        self._sumState_timeToStroke = StatCls.SummaryStat('Time until AIDS', self._times_to_Stroke)
